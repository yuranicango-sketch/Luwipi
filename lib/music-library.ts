...
export type { KidsSong, MusicRights, SongSection, PianoScoreEvent, ScoreHandMode } from "@/lib/music-types";
...
export function noteName(note:string){return note.replace(/[3-6]$/,"");}
export function noteOctave(note:string){const m=note.match(/([3-6])$/);return m?Number(m[1]):4;}
...
export function octaveAwareNotes(notes:string[],octaves:1|2|3=1){if(octaves===1)return notes;let octave=4,previous=-1;return notes.map(raw=>{if(/[3-6]$/.test(raw))return raw;const name=noteName(raw);const index=NAMES.indexOf(name as typeof NAMES[number]);if(previous>=0&&index>=0){if(previous-index>=4)octave=Math.min(5,octave+1);else if(index-previous>=4)octave=Math.max(4,octave-1);}previous=index;return `${name}${octave}`;});}
export function getSong(id:string){const song=kidsSongs.find((item)=>item.id===id);if(!song)return undefined;const octaves=song.pianoOctaves??1;if(octaves===1||song.scoreEvents?.length)return song;const sections=song.sections?.map(section=>({...section,notes:octaveAwareNotes(section.notes,octaves)}));return {...song,sections,sequence:sections?.flatMap(section=>section.notes)??octaveAwareNotes(song.sequence??[],octaves)};}
