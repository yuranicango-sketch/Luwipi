...
type Key={note:string;color:string;rate:number}; type Mode="site"|"piano"; type Theme="candy"|"star"|"paw"|"bell"|"water"|"music";
const base="https://tonejs.github.io/audio/salamander/C4.mp3";
const KEYS:Key[]=[
 {note:"Dó",color:"#ff5f86",rate:1},{note:"Ré",color:"#ffbf3f",rate:Math.pow(2,2/12)},{note:"Mi",color:"#64c96b",rate:Math.pow(2,4/12)},
 {note:"Fá",color:"#4fc8c1",rate:Math.pow(2,5/12)},{note:"Sol",color:"#4b9df8",rate:Math.pow(2,7/12)},{note:"Lá",color:"#8f74eb",rate:Math.pow(2,9/12)},{note:"Si",color:"#d264d7",rate:Math.pow(2,11/12)},
];
...
function sound(k:Key){const a=new Audio(base);a.preload="auto";a.playbackRate=k.rate;a.volume=.68;void a.play()}
...
function NoteToken({theme,color,note,active,done}:{theme:Theme;color:string;note:string;active:boolean;done:boolean}){
 return <div className={`token ${active?"active":""} ${done?"done":""}`}><svg viewBox="0 0 64 64" aria-hidden="true">
...
function Scene({song,progress}:{song:KidsSong;progress:number}){
 const meta=SPECIAL[song.id]??{theme:"music" as Theme,scene:"garden",finish:"História concluída!",goal:"🏁"};
 return <div className={`scene ${meta.scene}`}>
  ...
  <div className="path"/><span className="traveler" style={{left:`${5+progress*.82}%`}}>{song.emoji}</span><span className="goal">{meta.goal}</span>
 </div>
}

export function SongPractice({song}:{song:KidsSong}){
 const seq=song.sequence??[]; const piano=useMemo(()=>{const m=new Map(song.colors?.map(x=>[x.note,x.color])??[]);return KEYS.map(k=>({...k,color:m.get(k.note)??k.color}))},[song.colors]);
 const meta=SPECIAL[song.id]??{theme:"music" as Theme,scene:"garden",finish:"História concluída!",goal:"🏁"};
 const [started,setStarted]=useState(false),[mode,setMode]=useState<Mode>("site"),[step,setStep]=useState(0),[wrong,setWrong]=useState<string|null>(null),[listening,setListening]=useState(false); const token=useRef(0);
 const done=step>=seq.length, expected=done?undefined:seq[step], progress=seq.length?Math.min(100,Math.round(step/seq.length*100)):0;
 function reset(){token.current++;setStep(0);setWrong(null);setListening(false);setStarted(true)}
 function press(k:Key){sound(k);if(mode!=="site"||!expected)return;if(k.note!==expected){setWrong(k.note);setTimeout(()=>setWrong(null),350);return}setStep(v=>v+1)}
 async function listen(){if(listening)return;const id=++token.current;setListening(true);for(const n of seq){if(token.current!==id)return;const k=piano.find(x=>x.note===n);if(k)sound(k);await wait(470)}if(token.current===id)setListening(false)}
 if(!started)return <section className="card intro"><div className="heroEmoji">{song.emoji}</div><small>Historinha</small><h1>{song.title}</h1><p>{song.story}</p><button className="duo" onClick={()=>setStarted(true)}>COMEÇAR</button><style jsx>{css}</style></section>;
 if(done)return <section className="card intro"><div className="heroEmoji">🌟</div><h1>Muito bem!</h1><p>{meta.finish}</p><button className="duo" onClick={reset}>TOCAR DE NOVO</button><Link className="homework" href={`/professor/tarefas?song=${song.id}`}>Enviar como tarefa</Link><style jsx>{css}</style></section>;
 return <section className="card"><div className="head"><div><small>{step+1} de {seq.length}</small><h1>Agora: {expected}</h1></div><div className="mode"><button className={mode==="site"?"active":""} onClick={()=>setMode("site")}>No site</button><button className={mode==="piano"?"active":""} onClick={()=>setMode("piano")}>Piano físico</button></div></div>
  <Scene song={song} progress={progress}/><button className="listen" onClick={()=>void listen()}>{listening?"Tocando…":"🔊 OUVIR"}</button>
  <div className="notes">{seq.map((n,i)=>{const k=piano.find(x=>x.note===n)!;return <NoteToken key={`${n}-${i}`} theme={meta.theme} color={k.color} note={n} active={i===step} done={i<step}/>})}</div>
  {mode==="piano"?<div className="physical"><span>Toque no piano real</span><strong>{expected}</strong><button className="duo" onClick={()=>expected&&setStep(v=>v+1)}>TOCOU · CONTINUAR</button></div>:<div className="pianoShell"><div className="brand">LUWIPI PIANO</div><div className="pianoReal">{piano.map(k=><button key={k.note} onClick={()=>press(k)} className={`white ${expected===k.note?"expected":""} ${wrong===k.note?"wrong":""}`} style={{"--key":k.color} as CSSProperties}><span>{k.note}</span></button>)}<i className="black b1"/><i className="black b2"/><i className="black b3"/><i className="black b4"/><i className="black b5"/></div></div>}
  <style jsx>{css}</style></section>
}
