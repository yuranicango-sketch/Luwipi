...
export function LiveLesson({ offlineShell = false }: { offlineShell?: boolean } = {}) {
  ...
  if (finished) return <main className={styles.finishPage}>
    <section className={styles.finishCard}>
      ...
      <div className={styles.masteryGrid}>{focusCompetencies.map((id) => <div key={id} className={styles.masteryRow}><strong>{competencyLabels[id]}</strong><div>{masteryLevels.map((level) => <button key={level.id} aria-pressed={mastery[id] === level.id} data-active={mastery[id] === level.id} onClick={() => setMastery((current) => ({ ...current, [id]: level.id }))}>{level.label}</button>)}</div></div>)}</div>
  ...
  return <main className={`${styles.lesson} ${student?.reducedStimulus ? styles.reduced : ""}`} data-age={currentSession.ageBand}>
    ...
    {swap && <AccessibleModal label="Trocar bloco da aula" onClose={() => setSwap(false)} backdropClassName={styles.overlay} panelClassName={styles.sheet}><button className={styles.close} aria-label="Fechar" onClick={() => setSwap(false)}>×</button><span>TROCAR SÓ ESTE BLOCO</span><h2>Mesmo objetivo, outra forma.</h2><p>As primeiras opções preservam mais competências e a mesma modalidade. O restante da aula não muda.</p><div className={styles.choices}>{swapOptions.map((item) => <button key={item.id} onClick={() => replaceBlock(item)}><strong>{item.title}</strong><small>{item.durationLabel ?? `${item.minutes} min`} · {item.objective}</small></button>)}</div></AccessibleModal>}

    {wildcard && <AccessibleModal label="Escolher atividade coringa" onClose={() => setWildcard(false)} backdropClassName={styles.overlay} panelClassName={styles.sheet}><button className={styles.close} aria-label="Fechar" onClick={() => setWildcard(false)}>×</button><span>CORINGA · 60–90 SEGUNDOS</span><h2>Quebre o fluxo sem transformar isso numa recompensa.</h2><p>Use para tédio, perda de foco ou pequena falha técnica. Depois volte à aula.</p><div className={styles.choices}>{wildcardActivities[currentSession.ageBand].map((item) => <button key={item.id} onClick={() => chooseWildcard(item)}><strong>{item.title}</strong><small>{item.teacherCue}</small></button>)}</div></AccessibleModal>}

    {pause && <AccessibleModal label="Pausa e acolhimento" onClose={() => setPause(false)} backdropClassName={`${styles.overlay} ${styles.calmOverlay}`} panelClassName={styles.calm}><LessonGuide ageBand={currentSession.ageBand} kind="closing" reduced /><span>♡ PAUSA / ACOLHIMENTO</span><h2>Não precisamos continuar agora.</h2><p>Reduza estímulos. Baixe a voz. Dê espaço. A aula pode esperar.</p><div><span>Respirar juntos</span><span>Água / colo / silêncio</span><span>Sem contagem regressiva</span><span>Retomar só quando fizer sentido</span></div><button onClick={() => setPause(false)}>Voltar quando fizer sentido</button></AccessibleModal>}
  </main>;
}