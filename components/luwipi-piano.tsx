...
type Props={octaves?:1|2;expected?:string;wrong?:string|null;colors?:Map<string,string>;onPress?:(key:LuwipiPianoKey)=>void;showLabels?:boolean;compact?:boolean};

export function LuwipiPiano({octaves=1,expected,wrong,onPress,colors,showLabels=true,compact=false}:Props){
 ...
 return <div className={styles.shell} data-compact={compact?"true":"false"}><div className={styles.brand}>LUWIPI PIANO</div><div className={styles.scroll}><div className={styles.piano} style={{"--white-count":7*octaves} as CSSProperties}>
   {keys.map(key=>{const pitch=`${key.note}${key.octave}`;const active=expected===pitch||expected===key.note;return <button aria-label={pitch} key={pitch} type="button" onClick={()=>press(key)} className={`${styles.white} ${active?styles.expected:""} ${wrong===pitch?styles.wrong:""}`} style={{"--key":key.color} as CSSProperties}>{showLabels&&<span>{key.note}{octaves===2&&<small>{key.octave}</small>}</span>}</button>})}
   {Array.from({length:octaves},(_,o)=>BLACKS.map(([note,semitone,after])=>{const octave=o+4,pitch=`${note}${octave}`;const left=((o*7+after+1)/(7*octaves))*100;return <button aria-label={pitch} key={pitch} type="button" className={styles.black} style={{left:`calc(${left}% - ${octaves===2?"2.15%":"4.3%"} )`}} onClick={()=>{void playPianoRate(Math.pow(2,(semitone+o*12)/12))}}/>})).flat()}
 </div></div></div>
}