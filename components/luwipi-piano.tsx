...
type Props={octaves?:1|2;expected?:string;wrong?:string|null;colors?:Map<string,string>;onPress?:(key:LuwipiPianoKey)=>void;onBlackPress?:(pitch:string)=>void;showLabels?:boolean;compact?:boolean;disabled?:boolean;blackKeysInteractive?:boolean};

export function LuwipiPiano({octaves=1,expected,wrong,onPress,onBlackPress,colors,showLabels=true,compact=false,disabled=false,blackKeysInteractive=false}:Props){
 ...
 return <div className={styles.shell} data-compact={compact?"true":"false"}><div className={styles.brand}>LUWIPI PIANO</div><div className={styles.scroll}><div className={styles.piano} data-octaves={octaves} style={{"--white-count":7*octaves} as CSSProperties}>
   ...
   {Array.from({length:octaves},(_,o)=>BLACKS.map(([note,semitone,after])=>{const octave=o+4,pitch=`${note}${octave}`;const left=((o*7+after+1)/(7*octaves))*100;return <button aria-label={pitch} key={pitch} type="button" disabled={disabled||!blackKeysInteractive} className={styles.black} style={{left:`calc(${left}% - ${octaves===2?"2.15%":"4.3%"} )`}} onClick={()=>{void playPianoRate(Math.pow(2,(semitone+o*12)/12));onBlackPress?.(pitch)}}/>})).flat()}
 </div></div></div>
}