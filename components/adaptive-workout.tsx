...
export function AdaptiveWorkout({age,studentId="default"}:{age:AgeGroup;studentId?:string}){
 ...
 return <LearningPlayer backHref="/dashboard" eyebrow="TREINO ADAPTATIVO · SESSÃO ÚNICA" title={finished?"Treino concluído":mission?.title??"Treino 5 min"} progress={overall} status={finished?`${accuracy}%`:`${missionIndex+1}/${Math.max(1,plan.length)}`} toolbar={toolbar} tone="game" piano={{input,onInputChange:setInput,onExternalNote:externalPress,onPress:screenPress,onBlackPress:blackPress,expected:finished?undefined:expected,wrong,octaves:age==="2-4"?2:3,startOctave:age==="2-4"?4:3,showLabels:age!=="2-4",blackKeysInteractive:true,attentionCue:showCoach,hint:message}}>
