...
import {noteName,noteOctave,noteRate} from "@/lib/music-library";
...
export function PreschoolInlineSong({song,lessonNumber}:{song:KidsSong;lessonNumber:number}){
 ...
 function playKey(key:LuwipiPianoKey){if(!notes.length)return;const expected=notes[i%notes.length];if(key.note===noteName(expected)&&(song.pianoOctaves!==2||key.octave===noteOctave(expected)))advance();}
 ...
 return <div className={styles.wrap}>
  ...
  <LuwipiPiano compact octaves={song.pianoOctaves??1} expected={notes[i%notes.length]} onPress={playKey}/>
