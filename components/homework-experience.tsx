...
export function HomeworkExperience({ code }: { code: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/homework/${encodeURIComponent(code)}`,{cache:"no-store"})
      ...
      .catch(error=>{if(!cancelled){setLoadError(error instanceof Error&&error.message==="not_found"?"not_found":"load_failed");setAssignment(null);}});
  ...
  useEffect(() => {
    ...
    void syncProgress(assignment.code,true,false);
  }, [assignment]);
  ...
  async function syncProgress(assignmentCode:string,sessionStarted=false,noteCompleted=false,repetitionCompleted=false){
    try{
  ...
  function press(key: LuwipiPianoKey) {
    ...
    if (nextStep < sequence.length) {
      void syncProgress(assignment.code,false,true,false);
      setStep(nextStep);
      setMessage("Isso! Continue 🎵");
      return;
    }

    void syncProgress(assignment.code,false,true,true);
