...
export function CurriculumMap(){
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.methodPath}>
      ...
      <div className={styles.methodTabs}>{([1,2,3,4,5] as TrainingVolume[]).map((item)=><button key={item} data-active={trainingVolume===item} onClick={()=>setTrainingVolume(item)}>Vol. {item}</button>)}</div>
