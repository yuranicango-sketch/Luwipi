...
export function MusicScore({ notes, currentIndex, timeSignature = "4/4", wrongIndex = null, hideLabels = false, compact = false }: Props) {
  ...
  return <div className="scoreWrap" data-compact={compact ? "true" : "false"}>
    ...
    <style jsx>{`
      .scoreWrap{height:100%;min-height:${compact ? "150px" : "185px"};border:1px solid #dfe7ef;border-radius:20px;background:#f7fafc;padding:10px;overflow:hidden;display:grid;grid-template-rows:auto minmax(0,1fr)}.scoreTop{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:2px 5px 9px}.scoreTop div{display:flex;flex-direction:column;gap:2px}.scoreTop span{font-size:10px;font-weight:950;letter-spacing:.1em;color:#6270c7}.scoreTop b{font-size:12px;color:#64748b}.scoreTop strong{font-size:12px;padding:6px 8px;border-radius:9px;background:#eaf0f6;color:#3f5066}.scroll{min-height:0;overflow-x:auto;overflow-y:hidden;scrollbar-width:none;display:flex;align-items:center}.scroll::-webkit-scrollbar{display:none}.score{display:block;width:${width}px;max-width:none;height:100%;min-height:${compact ? "116px" : "140px"};max-height:${compact ? "145px" : "185px"}}.scoreWrap[data-compact=true]{padding:7px}.scoreWrap[data-compact=true] .scoreTop{padding-bottom:5px}.scoreWrap[data-compact=true] .scoreTop b{display:none}@media(max-width:680px){.scoreWrap{height:auto;min-height:0;display:block}.scoreTop b{display:none}.scroll{display:block}.score{height:${compact ? "116px" : "145px"}}`
    }</style>
  </div>;
}
