...
export function LessonRunner({age,program,module,lesson,variant,studentId,maxLesson=48,onLocked,onOpenMap,onNavigateLesson,onCompleted,onExit}:Props){
 ...
 if(step.gameId){
   ...
   content=<section className={styles.rhythmScene}><LessonScene age={age} lessonNumber={lesson.number} experience={sceneExperience} title={step.title} instruction={childPrompt}/><button className={styles.rhythmPad} onClick={()=>void rhythmTap()}><strong>TOQUE NO PULSO</strong><div>{[0,1,2,3].map(i=><i key={i} data-on={i<taps}/>)}</div></button></section>;
   ...
   content=<section className={styles.conceptScene}><LessonScene age={age} lessonNumber={lesson.number} experience={sceneExperience} title={step.title} instruction={childPrompt} expected={typeof expected==="string"?expected:undefined}/><div className={styles.missionBar}><div><small>{sceneExperience.gate==="teacher"?"PROFESSOR VALIDA":sceneExperience.gate==="free"?"CRIE NO PIANO":"TOQUE NO PIANO"}</small><strong>{message}</strong></div>{sceneExperience.gate==="teacher"?teacherValidate:<span data-done={done}>{done?"PRONTO":expected?noteName(Array.isArray(expected)?expected[0]:expected):`${freeNotes}/${policy.repetitions}`}</span>}</div></section>;
 }
