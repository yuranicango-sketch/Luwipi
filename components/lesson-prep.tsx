  ...
  return <div className={styles.workspace}>
    ...
    {students.length > 0 && <>
      <section className={styles.step}>
        ...
        <div className={styles.students}>{students.map((item) => <button key={item.id} onClick={() => { setStudentId(item.id); setShowPreferences(false); }} aria-pressed={item.id === studentId} data-active={item.id === studentId}><span className={styles.avatar}>{item.photoDataUrl ? <img src={item.photoDataUrl} alt="" /> : item.name.slice(0, 1).toUpperCase()}</span><span><strong>{item.name}</strong><small>{item.ageBand} anos · {item.level.replace("-", " ")}</small></span></button>)}</div>
      ...
      {student && <section className={styles.step}>
        ...
        <div className={styles.states}>{states.map((item) => <button key={item.id} onClick={() => setState(item.id)} aria-pressed={state === item.id} data-active={state === item.id}><b>{item.emoji}</b><strong>{stateLabel(item.id)}</strong><small>{item.description}</small></button>)}</div></div>
      </section>}
      ...
      {student && suggested && <section className={`${styles.step} ${styles.recommendationStep}`}>
        <div className={styles.stepNumber}>3</div><div className={styles.stepBody}>
          ...
          <article className={styles.lessonCard}>
            ...
            <div className={styles.instrument}><span>Instrumento hoje</span><div><button aria-pressed={instrumentMode === "physical"} data-active={instrumentMode === "physical"} onClick={() => setInstrumentMode("physical")}>🎹 Piano físico</button><button aria-pressed={instrumentMode === "virtual"} data-active={instrumentMode === "virtual"} onClick={() => setInstrumentMode("virtual")}>▤ Piano virtual</button><button aria-pressed={instrumentMode === "silent"} data-active={instrumentMode === "silent"} onClick={() => setInstrumentMode("silent")}>◌ Modo silencioso</button></div></div>
