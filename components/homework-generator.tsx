...
export function HomeworkGenerator() {
  ...
  function generate() {
    ...
    const assignment = {
      code,
      childName: childName.trim() || "Pequeno músico",
      songId,
      teacherNote: teacherNote.trim(),
      targetRepeats,
      validUntil: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    };
    localStorage.setItem(`luwipi-homework:${code}`, JSON.stringify(assignment));
    ...
    <div className="homework-generator">
      <div className="homework-form-grid">
        <label>
          ...
          <input value={childName} onChange={(event) => setChildName(event.target.value)} placeholder="Ex.: Maria" />
        </label>
        <label>
