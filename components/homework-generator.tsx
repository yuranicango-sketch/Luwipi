...
export function HomeworkGenerator({ initialSongId }: { initialSongId?: string }) {
  ...
  function musicHref(id:string){
    if(!selected?.id)return `/musicas/${id}`;
    return `/context?student=${encodeURIComponent(selected.id)}&next=${encodeURIComponent(`/musicas/${id}`)}`;
  }
