...
export function StudentManager(){
  ...
  const [error,setError]=useState(""),[busy,setBusy]=useState(false),[driveDisconnected,setDriveDisconnected]=useState(false);
  ...
  async function refresh(){
    ...
    try{
      ...
      setStudents(s.students??[]);setGroups(g.groups??[]);setDriveDisconnected(false);
    }catch(e){
      const code=e instanceof Error?e.message:"request_failed";
      setDriveDisconnected(code==="drive_not_connected"||code==="drive_token_failed"||code==="drive_encryption_not_configured");
      setError(code.startsWith("drive_")?"O Google Drive precisa ser ligado novamente para guardar alunos e turmas.":"Não foi possível carregar alunos e turmas.");
    }
  }
  ...
  async function mutate(url:string,body:unknown){
    ...
    catch(e){const code=e instanceof Error?e.message:"request_failed";setDriveDisconnected(code.startsWith("drive_"));setError(code.startsWith("drive_")?"Volte a autorizar o Google Drive e tente novamente.":"Não foi possível guardar a alteração.");return false}
