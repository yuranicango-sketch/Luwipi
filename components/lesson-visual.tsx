...
export function LessonVisual({stepId,title,age,accent,instruction=""}:Props){
 ...
 const[stored,setStored]=useState<StoredAsset[]>([]),[loaded,setLoaded]=useState(false),[canReview,setCanReview]=useState(false),[busy,setBusy]=useState<string|null>(null),[error,setError]=useState(""),ref=useRef<HTMLDivElement>(null),[active,setActive]=useState(0);
 async function reload(){setLoaded(false);if(!resolved){setStored([]);setCanReview(false);setLoaded(true);return}try{
  const response=await fetch(`/api/pedagogy-assets?visual_key=${encodeURIComponent(resolved)}`,{cache:"no-store"});
  const body=await response.json().catch(()=>({}));
  if(!response.ok)throw new Error(body.error||"Falha ao carregar imagens");
  setStored((body.assets??[]) as StoredAsset[]);
  setCanReview(Boolean(body.canReview));
 }catch(e){setStored([]);setCanReview(false);setError(e instanceof Error?e.message:"Falha ao carregar imagens")}finally{setLoaded(true)}}
 useEffect(()=>{setActive(0);setError("");void reload()},[resolved,instruction]);
 const approved=stored.filter(x=>x.status==="approved").map(x=>({src:x.public_url??`/api/pedagogy-assets/${x.id}/preview`,alt:x.alt_text,focus:x.focus,source:"Luwipi",license:"approved"}));
 const candidate=canReview?stored.find(x=>x.status==="candidate"):undefined;
