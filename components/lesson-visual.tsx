...
function CandidateImage({asset}:{asset:StoredAsset}){const[e,setE]=useState(false);useEffect(()=>setE(false),[asset.source_url]);if(e)return <div className={styles.imageFallback}><b>Fonte da imagem indisponível</b><span>Reprove para procurar outra opção.</span></div>;return <img className={styles.realAsset} src={asset.source_url} alt={asset.alt_text||"Imagem pedagógica candidata"} loading="eager" onError={()=>setE(true)}/>;}
