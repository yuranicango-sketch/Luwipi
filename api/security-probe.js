...
async function record(request,user,trap){
  ...
  await fetch(c.url+"/rest/v1/security_honeypot_events",{
    ...
    body:JSON.stringify({
      ...
      weight:trap.weight,
      user_agent:ua||null,
      request_method:request.method.slice(0,12),
      request_path:url.pathname.slice(0,200)
    })
  });
}
async function handle(request){
  const url=new URL(request.url);
  const key=url.searchParams.get("trap")||url.pathname.split("/").filter(Boolean).pop()||"";
